...
double peek_stack(void);

void store_variable(char var, double val);
double load_variable(char var);
  ...
  while ((c = get_from_input(s)) != TOKEN_EOF) {
    switch (c) {
      ...
      case TOKEN_DIVIDE:
        ...
        break;
      case TOKEN_STORE:
        if ((c = get_from_input(s)) == TOKEN_VAR) {
          store_variable(s[0], peek_stack());
        } else {
      ...
      case TOKEN_LOAD:
        if ((c = get_from_input(s)) == TOKEN_VAR) {
          push_onto_stack(load_variable(s[0]));
        } else {
          printf("error: expected variable name after '>'\n");
        }
        break;
      case TOKEN_NEWLINE:
        store_variable('&', peek_stack());
        printf("\t%.8g\n", pop_from_stack());
        break;
      default:
...
/* 26 variables with single letter names (alphabet letters) + 1 at the end which
 * holds the latest value printed, accessible with '&'. */
#define NUM_VARIABLES 27

double vars[NUM_VARIABLES];
bool vars_initialized[NUM_VARIABLES] = {false};

void store_variable(char var, double val) {
  if (var == '&') {
    vars[NUM_VARIABLES - 1] = val;
    vars_initialized[NUM_VARIABLES - 1] = true;
  } else {
    int var_index = var - 'a';

    if (var_index >= 0 && var_index < NUM_VARIABLES - 1) {
      vars[var_index] = val;
      vars_initialized[var_index] = true;
    } else {
...
double load_variable(char var) {
  if (var == '&') {
    if (vars_initialized[NUM_VARIABLES - 1]) {
      return vars[NUM_VARIABLES - 1];
    } else {
