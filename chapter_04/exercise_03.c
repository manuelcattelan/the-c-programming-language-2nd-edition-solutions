  ...
  while ((c = get_from_input(s)) != EOF) {
    switch (c) {
      ...
      case '%':
        second_operand = pop_from_stack();
        if (second_operand != 0.0) {
          push_onto_stack(fmod(pop_from_stack(), second_operand));
        } else {
          printf("error: zero divisor\n");
        }
        break;
...
int get_from_input(char s[]) {
  ...
  if (c == '-') {
    /* Handle negative numbers by checking if - is immediately followed by a
     * digit, in which case we store the first and read the rest below. */
    int c_next = get_char();
    if (isdigit(c_next) || c_next == '.') {
      ...
      /* If the - character is not followed by a digit, we unget the character
       * we read for negative number checking and return '-' as an operator. */
      unget_char(c_next);
      return '-';
    }
  } else if (!isdigit(c) && c != '.') {
    ...
    /* The character is a positive digit and we initialize i so that we can
