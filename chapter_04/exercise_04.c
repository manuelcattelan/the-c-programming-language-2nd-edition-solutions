  ...
  while ((c = get_from_input(s)) != TOKEN_EOF) {
    switch (c) {
      ...
      case TOKEN_DUPLICATE:
        push_onto_stack(peek_stack());
        break;
      case TOKEN_SWAP: {
        double first_val = pop_from_stack();
        double second_val = pop_from_stack();
        push_onto_stack(second_val);
        push_onto_stack(first_val);
        break;
      }
      case TOKEN_CLEAR:
        clear_stack();
...
void clear_stack(void) { stack_pointer = 0; }
